...
class User_lib
{
    ...
    public function getAllNotificationDESC($user_ID){
        $query = $this->CI->db->select("*,notifications.status as noti_status,notifications.created_on as notify_date")
                        ->from("notifications")
    ...
    Public function timeAgo($datetime, $full = false) {
        ...
        if (!$full) {
            $result = array_slice($result, 0, 1); // just the most significant part (e.g., '2 hours')
        }
